...
void TimerXIntHandler(void)
{
    ...
    {
        flag = 1;
        TimerDisable(TIMERX_BASE, TIMER_A);
        admemNum = 0;
    }
...
void TimerInt_Init(void)
{
    ...
    TimerConfigure(TIMERX_BASE, TIMER_CFG_X);																	    //定时器的高层配置，这里设为全字长周期定时
    //ui32Period = (SysCtlClockGet() / TIMER_INTERVAL_NUM) / 2;											//SysCtlClockGet()返回系统时钟频率（也是除了PWM外其他模块的频率）
    TimerLoadSet(TIMERX_BASE, TIMER_AorB, TimerLoadSetNUM);													//设置定时器重装载值（第二个参数指定的要调整的定时器，全字长周期只能写TIMER_A）
    TimerIntRegister(TIMERX_BASE,TIMER_AorB,TimerXIntHandler);										//注册中断，将中断触发和中断服务函数联系起来（实质上是修改.s中的中断向量表，第二个参数是函数指针，也可以手动注册）
    TimerIntEnable(TIMERX_BASE, TIMER_TIMAorB_INTx);															//使能单个的定时器中断源
    IntEnable(INT_TIMERXAorB);																										//使能中断
    IntMasterEnable();																														//这个函数允许处理器响应中断。不影响中断控制器的中断使能设置
    TimerEnable(TIMERX_BASE, TIMER_A);
}
...
void loop() {
  ...
  int type1 = A.sin;
  int type2 = B.sin;
  currentButtonState = digitalRead(OPEN_PIN);
  if (currentButtonState == LOW && previousButtonState == HIGH) {
  if(old1 != freq1 || old2 != freq2 || old3 != type1 || old4 != type2){
